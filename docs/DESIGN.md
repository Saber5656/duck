...
| 入れる | メニューバー常駐 + **Listening を 1 クリックで on/off** | off はマイク完全解放（エンジン停止）。オレンジインジケータ消灯と同期（§7.3） |
...
- **権限フロー**: `Info.plist` に `NSMicrophoneUsageDescription` が必須（ないと即クラッシュ）。
  ...
  **sandbox 下では audio-input entitlement がないと権限プロンプト自体が出ない**ため、entitlement 設定が最優先の検証項目。
- **entitlement**: App Sandbox のマイクは `com.apple.security.device.audio-input`（Xcode の "Audio Input"）。
...
- **listening 中は寝ない。** sleep は「マイクを開いていない」ことの視覚化専用とし、
  オレンジインジケータ消灯と duck の就寝を常に同期させる（§7.3 の検証可能性の一部）。
...
- 配置は四隅から選択（既定 Bottom Right）。`NSScreen.main.visibleFrame` 基準 + 16pt オフセットで Dock とメニューバーを避ける。移動しないので tick 内の窓操作はゼロ
...
3. 許可 → duck が選択した隅に登場して挨拶の nod。同時に
   「メニューバーのオレンジの点は duck が聞いている印です。Listening を切ると消えます」と 1 文表示（チュートリアルはこれで終わり）
...
| **見ないもの** | 波形の保存・コピー、内容を復元しうる特徴量（スペクトログラム・MFCC・ピッチ系列）、音声認識（STT）、話者識別、他アプリの音声 |
...
4. **層 4 — 禁止 API の CI ガード**: `SFSpeechRecognizer` / `AVAudioFile` / `AVAudioRecorder` / `AVCaptureAudioFileOutput` /
   `URLSession` / `Network.framework` / `NWConnection` / `socket(` などを `git grep` で検出したら CI を fail させる（privacy-guard）。
   「守っていること」をバッジで常時証明する。
...
- **消せない・消さない。** README とオンボーディングで「点いている＝duck が聞いている、が正しい状態」と最初に説明する
- **Listening off は 1 クリック**（メニューバー直下の最上段）。off は `AVAudioEngine.stop()` + tap 除去による
  **マイクの完全解放**であり、ミュートやフラグではない。→ **off にするとオレンジドットが消える**
- duck の sleep アニメとオレンジドット消灯を常に同期させる。ユーザーは
  「duck が寝ている ⇔ 点が消えている ⇔ OS がマイク未使用と保証している」を自分の目で照合できる。
...
- **App Sandbox を有効化し、entitlement は `com.apple.security.device.audio-input` のみ**。
...
- **OS 側**: §7.3 のオレンジドット照合手順

### 7.7 マイク共有と会議

macOS のマイクは排他ではなく複数アプリで共有できるため、会議中も duck は動作しうる。
このとき duck が「聞く」のは会議の音声そのものだが、受け取り得るのは音量 1 値のみであり、内容・話者は原理的に知り得ない。
...
duck opens your microphone, so here is exactly what it does with it:
  ...
  |------------------------------------------------|--------------------------------------|
  | the loudness of each audio buffer (one number) | speech-to-text / recording / storing |
...
- **The macOS orange mic dot stays on while duck listens. That's honesty, not a bug.**
  Toggle "Listening" off in the menu bar and the dot goes out, because duck fully
  releases the microphone. The OS indicator is your independent proof.
...
| 優先度 | 項目 | 内容 / 検証方法 |
|---|---|---|
| **P0** | sandbox + audio-input entitlement での AVAudioEngine 動作一式 | entitlement（audio-input / 旧 microphone キーの要否）→ TCC プロンプト表示 → 許可 → tap で RMS 取得 → stop でオレンジドット消灯、の一連を実機確認。ここが崩れると §7 の根幹（sandbox 有効）が崩れる → spike #1 |
...
- **#1 `Spike: volume-only VAD with AVAudioEngine in a sandboxed app`** — ラベル: `spike`, `design`
  sandbox + audio-input entitlement + network entitlement なしの最小アプリで、TCC プロンプト → installTap → vDSP RMS → speaking/silence 判定のコンソール出力までを捨てプロトで作り、P0 リスク 2 件を検証する。実発話・タイピング・BGM で attack/hangover/閾値の初期値を決め、entitlement の要否（audio-input / microphone）と stop 時のオレンジドット消灯を確認する。
...
- **#2 `Set up menu bar app skeleton with sandbox and privacy-guard CI`** — ラベル: `infra`
  `LSUIElement` の Swift アプリ雛形、NSStatusItem メニュー（Listening トグル仮 / Quit / Launch at Login）、App Sandbox 有効・network entitlement なし、UserDefaults 永続化、GitHub Actions の build + 禁止シンボル grep（privacy-guard）を整備する。
...
- **#3 `Implement AudioLevelSource with discard-only RMS tap`** — ラベル: `enhancement`
  ...
  受け入れ条件: start で RMS が流れ stop でオレンジドットが消灯する。バッファ参照がコールバック外に出ないことをレビューで確認。AirPods 着脱で動作継続。
...
- **#7 `Add onboarding and permission flow with sleeping demo mode`** — ラベル: `ux`
  初回起動ウィンドウ（10 秒説明 + プライバシー要約 + Start listening）、`AVCaptureDevice.requestAccess`、拒否時の sleeping デモモード（Nod once 可・⚠ バッジ・設定ディープリンク）、オレンジドットの 1 文説明、Listening 状態の復元を実装する。
...
- マイク権限と sandbox entitlement（audio-input entitlement、sandbox 下では entitlement なしだとプロンプト自体が出ない）:
